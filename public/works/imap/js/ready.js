(function () {
	...
	function ready(error, map, Russia, dictionary) {
		...
		function showDistrictsWithCities () {
			d3.selectAll('#map-Russia > .markers .with_cityes_marker').each(function (data,index) {
				var marker = d3.select(this)
				marker
					.attr({
						'transform' : function (d) {
							return 'translate('+ (d[0] - 10) +','+(d[1] -25 -100) +')';
						}
					})
					.select('.with_cityes_marker_image')
					.style({
						'display' : 'block'
					})
					.transition()
				...
				marker
					...
					.attr({
						'transform' : function (d) {
							return 'translate('+ (d[0] - 10) +','+(d[1] -25) +')';
						}
					})
			})
		};
		function hideDistrictsWithCities () {
			d3.selectAll('#map-Russia > .markers .with_cityes_marker > .with_cityes_marker_image')
				.transition()
				.duration(baseDuration*2)
				.attr({
					'transform' : function (d) {
		...
		function showRegionsWithCities (element) {
			element.selectAll('.markers .with_cityes_marker').each(function (d,index) {
				var marker = d3.select(this)
				var coords = d.coords
				marker
					.attr({
				...
				marker
					...
					.attr({
						transform : function (d) {
							return  'translate('+ (coords[0])+',' + (coords[1]) +')' + 'scale('+( 1/ d3.select(this.parentNode.parentNode).datum().zoomValue)+')'
						}
					})
			})
		};
		function hideRegionsWithCities (element) {
			...
			svg.selectAll('.district.with_cities > .markers .with_cityes_marker_image')
				.transition()
				.duration(baseDuration*2)
				.style('opacity',0)
				.delay(10)
						...
						if ( city.size() > 0 ) {
							...
							if ( d3.select(this).classed('with_cities') === false ) {
								...
								d3.select(this)
									.append('g')
									.attr({
										'class': 'with_cityes_marker',

									})
									.append("image")
									.attr({
										'class' : 'with_cityes_marker_image'
									})
									.attr("xlink:href", "sources/red-marker.png")
									.attr("width", 42)
									.attr("height", 42)
		...
		svg.selectAll('.region.with_cities .with_cityes_marker').each(function (d) {
		...
		var zoomOutButton = svg.append("image")
			.attr({
				...
				'xlink:href': 'sources/zoom-out.png',
				'width': 42,
				'height': 42
			})
			.style({
